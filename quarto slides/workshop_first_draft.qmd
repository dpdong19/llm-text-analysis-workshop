...
2. **Download the project folder** from [github.com/dpdong19/llm-text-analysis-workshop](https://github.com/dpdong19/llm-text-analysis-workshop): green **Code** button → **Download ZIP** → unzip it
...
::: {.notes}
Leave this up while people arrive. Helpers walk around.
:::
...
::: {.notes}
Many people are installing Python or a coding agent for the first time today. Lower the pressure: watching is a perfectly good way to take part.
:::
...
::: {.notes}
Today we stop at the classification step, but this is the reason we do it. Questions 2 and 3 need the metadata we removed earlier: join on review_id.
:::
...
On supervised ML: LLMs remove the need for a large training set, but not for a gold standard. It moves from training to checking; our reference set plays that role. Supervised ML still wins at very large scale, when strict reproducibility matters, or when data cannot leave your machine. A common hybrid: label a sample with an LLM, then train a small model on it.
:::
...
::: {.notes}
Colleagues send the shared key individually. If many 429 / rate-limit errors appear, someone has probably parallelised their script.
:::
...
::: {.notes}
For fast participants during the compare step. If time allows, show one chart on screen before the Menti quiz.
:::
